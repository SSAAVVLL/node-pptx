...
class Presentation {
    ...
    zipContent() {
        ...
        let contentOrigin = this.contentOrigin;

        // console.log(contentOrigin['ppt/slides/slide8.xml']);
        for (let key in content) {
            if (content.hasOwnProperty(key)) {
                ...
                if (ext === '.xml' || ext === '.rels') {
                    ...
                    if (key.search(/.*slideLayouts.*/) !== -1) {
                        xml = contentOrigin[key];
                    } else {
                        console.log('Key:', key);
                        // console.log(xml);
                        let regEx = /.*?<a:custGeom>((.|\n)*)<\/a:custGeom>.*?/gm; 
                        let originGeoms = regEx.exec(contentOrigin[key]);
                        if (originGeoms !== null) {
                            // xml = xml.replace(/(\r\n|\n|\r)/gm," ");
                            // console.log(xml);
                            let geoms = regEx.exec(xml);
                            let geomText = originGeoms[1];
                            // console.log(contentOrigin[key]);
                            // console.log(xml);
                            // fs.writeFileSync('test.txt', xml);
                            // fs.writeFileSync('testOrigin.txt', contentOrigin[key]);
                            // console.log(geomText);
                            console.log(geoms);
                            
                            // xml = xml.replace(geoms[1], geomText);
                        }
                    }
